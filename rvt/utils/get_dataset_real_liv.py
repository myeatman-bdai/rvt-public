...
import clip
import torch
from rvt.utils.dataset_real_liv import create_replay, fill_replay_real
...
CAMERAS_REAL = ["camera1", "camera2", "camera3"]
...
def load_liv_custom_ckpt(path, modelid='resnet50'):
    ...
    cleancfg = cleanup_config(modelcfg)
    rep = hydra.utils.instantiate(cleancfg)
    state_dict = torch.load(path, map_location=torch.device("cpu"))['liv']
...
def get_dataset_real(
    ...
    try:
        ...
        if liv_dir is None:
            print("WARNING: Loading default LIV checkpoint.")
            liv_model = load_liv()
        else:
            liv_model = load_liv_custom_ckpt(liv_dir)
    ...
    for task in tasks:  # for each task
        ...
        EPISODES_FOLDER_TRAIN = f"rw/train/{task}"
        EPISODES_FOLDER_VAL = f"rw/val/{task}"
        data_path_train = os.path.join(DATA_FOLDER, EPISODES_FOLDER_TRAIN)
        data_path_val = os.path.join(DATA_FOLDER, EPISODES_FOLDER_VAL)
        train_replay_storage_folder = f"{TRAIN_REPLAY_STORAGE_DIR}/rw/{task}"
        test_replay_storage_folder = f"{TEST_REPLAY_STORAGE_DIR}/rw/{task}"
