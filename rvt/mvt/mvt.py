...
class MVT(nn.Module):
    ...
    def render(self, pc, img_feat, img_aug, dyn_cam_info):
        ...
        with torch.no_grad():
            ...
            # print("##", img.shape)
            # tmp = img[0, :, 3: 6, :, :].detach().cpu().numpy()
            # for i, tmp2 in enumerate(tmp):
            #     tmp2 = tmp2.transpose((1, 2, 0))
            #     print(tmp2.max(), tmp2.dtype)
            #     from PIL import Image
            #     import numpy as np
            #     Image.fromarray((tmp2 * 255).astype(np.uint8)).save(f"tmp_{i}.png")
