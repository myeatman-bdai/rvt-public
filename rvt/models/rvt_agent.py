...
class RVTAgent:
    def __init__(
        ...
        self.liv_tau = liv_tau
        self.liv_reward_index = liv_reward_index
    ...
    def update(
        ...
        if backprop:
            # cross-entropy loss
            trans_loss = self._cross_entropy_loss(q_trans, action_trans).mean(dim=1)#.mean()
            ...
            weight = None
            if "rewards" in replay_sample:
                rewards = replay_sample["rewards"]
                # print("!!", rewards.shape)
                reward = rewards[:, 0, self.liv_reward_index]
                weight = torch.exp(self.liv_tau * reward)
                # print("@@", total_loss.shape, weight.shape)
                # print(weight)
                total_loss = total_loss * weight.detach()

            total_loss = total_loss.mean()
            trans_loss = trans_loss.mean()
            if self.add_rgc_loss:
                rot_loss_x = rot_loss_x.mean()
                rot_loss_y = rot_loss_y.mean()
                rot_loss_z = rot_loss_z.mean()
    ...
    @torch.no_grad()
    def act(
        self, step: int, observation: dict, deterministic: bool=True, pred_distri: bool=False, show_pcds: bool=False
    ) -> ActResult:
        if self.add_lang:
        ...
        if show_pcds:
            ...
            print("move pc in bound:", self.move_pc_in_bound)
            print("place with mean:", self._place_with_mean)
            import open3d as o3d    
            print("1", pc.shape)
            o3d_pcd = o3d.geometry.PointCloud()
            o3d_pcd.points = o3d.utility.Vector3dVector(pc[0].cpu().numpy())
