...
def plot_curve(embeddings, keypoints, goal_embedding_text, task, model, imgs_tensor, save_dir, name, tau=0.1):
    ...
    for t in range(embeddings.shape[0]):
        ...
        s_obs_to_goal = model.module.sim(cur_embedding, embeddings[-1]).detach().cpu().numpy()
        s_key_to_goal = model.module.sim(embeddings[find_next_keypoint(t, keypoints)], embeddings[-1]).detach().cpu().numpy()

        cur_distance_img = s_key_to_goal - s_obs_to_goal
...
def main(args):
    ...
    save_path = f"/home/obiza/reward_curves_rwr/tau_{args.tau}/{args.task_path.split('/')[-1]}/episode{args.episode}"
