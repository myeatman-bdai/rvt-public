...
def plot_curve(embeddings, keypoints, goal_embedding_text, task, model, imgs_tensor, save_dir, name):
    ...
    for t in range(embeddings.shape[0]):
        ...
        cur_distance_img = - model.module.sim(embeddings[find_next_keypoint(t, keypoints)], cur_embedding).detach().cpu().numpy()
...
def main(args):
    ...
    save_path = f"/home/obiza/reward_curves_keyframes/{args.task_path.split('/')[-1]}/episode{args.episode}"
